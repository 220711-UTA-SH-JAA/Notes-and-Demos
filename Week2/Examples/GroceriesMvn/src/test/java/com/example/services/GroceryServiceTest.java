...
//		groceryService = new GroceryService(mockRepo);
	...
	public void testGetAllItems() {
		...
		assertEquals(new GroceryItem(1, "Test", 1.99, "TestType"), actualItems.get(0));
	}
	
	@Test
	public void testAddItem() {
		GroceryItem expected = new GroceryItem(1, "Test", 1.99, "TestType");
		//When we call mockRepo.saveItem with any object
		when(mockRepo.saveItem(any())).thenReturn(expected);
		
		GroceryItem actual = groceryService.createNewItem("Test", 1.99 , "TestType");
	...
	public void testGetItemExists() {
		...
		Collection<GroceryItem> collection = new ArrayList<>();
		collection.add(new GroceryItem(1, "Test", 1.99, "TestType"));
		collection.add(new GroceryItem(2, "Test2", 2.99, "TestType2"));
		collection.add(new GroceryItem(3, "Test3", 3.99, "TestType3"));
		
		when(mockRepo.getAllItems()).thenReturn(collection);
		
		GroceryItem actualItem = groceryService.getItemByName("Test2");
		
		assertEquals(new GroceryItem(2, "Test2", 2.99, "TestType2"), actualItem);
	}
	
	//So we can actually test whether an expected exception was thrown in Junit
	@Test(expected=ItemNotFoundException.class)
	public void testGetItemNotExist() {
		//We can use some of the static methods that come with mockito to explain/describe what we want to happen when
		//a mocked object gets called
		Collection<GroceryItem> collection = new ArrayList<>();
		collection.add(new GroceryItem(1, "Test", 1.99, "TestType"));
		...
		GroceryItem actualItem = groceryService.getItemByName("Test4");
		
		//This will never run, because it automatically passes when it sees the exception we expected
		//The assertTrue would technically fail, but the test saw the exception we expected and passed
		assertTrue(false);
	}
